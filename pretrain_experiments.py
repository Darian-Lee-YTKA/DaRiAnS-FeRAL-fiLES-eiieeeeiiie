...
def run_model(n_embd, n_head, n_layer, train_loader, val_loader, pretrain_encoder_path, pretrain_decoder_path, train_encoder_path=None, train_decoder_path=None, train=False):
    ...
    for epoch in range(num_epoch):
        ...
        for batch in train_loader:
            ...
            decoder_outputs = decoder(decoder_input, hidden_states, encoder_inputs, encoder_entity_embeddings=encoder_entity_embeddings, entity_info=None, use_encoders_entities=True, entities_in_self_attn=False, entity_info_for_pad_mask =entity_info) # NOTE: the encoder returns the embeddings it used for entities
            ...
            if random_int == 12: # so that we are printkng occasionally but not overwhelming the log
        ...
        with torch.no_grad():
            for batch in val_loader:
                if len(batch) == 4: # we don;t have any entity info
                    ...
                    encoder_input, decoder_input, target, mask, entity_info = batch
                    print("we have entity info in val")
        ...
        if train:
            ...
            name = "results_" + str(epoch) + ".csv"

            df = pd.DataFrame({'target': target_sentences, 'predicted': pred_sentences})
...
def test_model(n_embd, n_head, n_layer, test_loader, train_encoder_path=None, train_decoder_path=None):
    print("oh boy ")
    pos = PositionalEmbedding(n_embd)
    encoder = TransformerEncoder(vocab_size=vocab_size,
                                n_embd=n_embd,
    ...
    encoder.load_state_dict(torch.load(train_encoder_path, weights_only=True))
    decoder.load_state_dict(torch.load(train_decoder_path, weights_only=True))
    ...
    for batch in test_loader:
            ...
            encoder_input, decoder_input, target, mask, entity_info = batch
            # we are just not gonna use target
            ...
            decoder_outputs = decoder.generate(decoder_input, hidden_states, encoder_inputs, encoder_entity_embeddings=encoder_entity_embeddings, entity_info=None, use_encoders_entities=True, entities_in_self_attn=False) # NOTE: the encoder returns the embeddings it used for entities
